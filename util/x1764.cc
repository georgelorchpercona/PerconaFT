...
uint32_t toku_x1764_memory_simple(const void *buf, int len) {
    ...
    while (len >= 8) {
        c = c * 17 + *lbuf;
        if (PRINT)
            printf("%d: c=%016" PRIx64 " sum=%016" PRIx64 "\n",
                   __LINE__,
                   *lbuf,
                   c);
        lbuf++;
...
void toku_x1764_add(struct x1764 *l, const void *vbuf, int len) {
    ...
    if (n_input_bytes) {
        ...
        if (len >= 8) {
            ...
            while (len >= 8) {
                ...
                sum = sum * 17 + input;
                if (PRINT)
                    printf("%d: input=%016" PRIx64 " sum=%016" PRIx64 "\n",
                           __LINE__,
                           input,
                           sum);
                input = thisv >> (8 * (8 - n_input_bytes));
                if (PRINT)
                    printf("%d: input=%016" PRIx64 "\n", __LINE__, input);
                len -= 8;
                cbuf += 8;
                // n_input_bytes remains unchanged
                if (PRINT)
                    printf("%d: n_input_bytes=%d len=%d\n",
                           __LINE__,
                           l->n_input_bytes,
                           len);
            }
        ...
        if (len >= 4) {
            ...
            if (n_input_bytes < 4) {
                ...
                n_input_bytes -= 4;
                if (PRINT)
                    printf("%d: input=%016" PRIx64 " n_input_bytes=%d\n",
                           __LINE__,
                           input,
                           n_input_bytes);
            }
    ...
    if (1) {
        ...
        switch (len) {
            case 7:
                input = ((uint64_t)(*(uint32_t *)(cbuf))) |
                        (((uint64_t)(*(uint16_t *)(cbuf + 4))) << 32) |
                        (((uint64_t)(*(cbuf + 4))) << 48);
                break;
            case 6:
                input = ((uint64_t)(*(uint32_t *)(cbuf))) |
        ...
        if (len >= 2) {
            input |= ((uint64_t)(*(uint16_t *)(cbuf))) << (i * 8);
            cbuf += 2;
            len -= 2;
            i += 2;
        }
        if (len >= 1) {
            input |= ((uint64_t)(*(uint8_t *)(cbuf)))
                     << (i * 8); /*cbuf+=1; len-=1; i++;*/
        }
    }
    l->input = input;
